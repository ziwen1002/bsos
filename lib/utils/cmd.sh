...
# shellcheck source=/dev/null
source "${SCRIPT_DIR_e53d23f3}/unit_test.sh"
...
function cmd::_test_simple_cmd() {
    ...
    unit_test::assert_equal "$output" "$test_str"
}
...
function cmd::_test_run_cmd() {
    ...
    println_success "all test passed"
    return "$SHELL_TRUE"
}
