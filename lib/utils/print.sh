...
function println_black() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_BLACK}" "" "$1\n" "${@:2}"
}

# 前景色是红色
function println_red() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_RED}" "" "$1\n" "${@:2}"
}

# 前景色是绿色
function println_green() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_GREEN}" "" "$1\n" "${@:2}"
}

# 前景色是黄色
function println_yellow() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_YELLOW}" "" "$1\n" "${@:2}"
}

# 前景色是蓝色
function println_blue() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_BLUE}" "" "$1\n" "${@:2}"
}

# 前景色是紫色
function println_purple() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_PURPLE}" "" "$1\n" "${@:2}"
}

# 前景色是深绿
function println_dark_green() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_DARK_GREEN}" "" "$1\n" "${@:2}"
}

# 前景色是白色
function println_white() {
    printf_style "${P_DISPLAY_MODE_HIGHLIGHT}" "${P_FOREGROUND_WHITE}" "" "$1\n" "${@:2}"
}
