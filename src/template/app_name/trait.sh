...
# - 更新的指责和包管理器类似，只负责更新
function template::trait::upgrade() {
    package_manager::upgrade "$(template::trait::package_manager)" "$(template::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 如果直接卸载也不会有残留就不用处理
function template::trait::unfixme() {
    return "${SHELL_TRUE}"
}
