...
# 安装的操作
function thunderbird::trait::install() {
...
function thunderbird::trait::post_install() {
    hyprland::config::add "350" "${SCRIPT_DIR_5d9b31a0}/thunderbird.conf" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 卸载的操作
function thunderbird::trait::uninstall() {
...
# - 更新的指责和包管理器类似，只负责更新
function thunderbird::trait::upgrade() {
    package_manager::upgrade "$(thunderbird::trait::package_manager)" "$(thunderbird::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
