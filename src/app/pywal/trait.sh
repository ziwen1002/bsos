...
SCRIPT_DIR_61e35e60="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
# 指定使用的包管理器
function pywal::trait::package_manager() {
    echo "pacman"
}

# 需要安装包的名称，如果安装一个应用需要安装多个包，那么这里填写最核心的包，其他的包算是依赖
function pywal::trait::package_name() {
    echo "python-pywal"
}

# 简短的描述信息，查看包的信息的时候会显示
function pywal::trait::description() {
    package_manager::package_description "$(pywal::trait::package_manager)" "$(pywal::trait::package_name)" || return "$SHELL_FALSE"
    return "$SHELL_TRUE"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function pywal::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function pywal::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的前置操作，比如卸载依赖
function pywal::trait::pre_uninstall() {
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function pywal::trait::upgrade() {
    package_manager::upgrade "$(pywal::trait::package_manager)" "$(pywal::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
function pywal::trait::dependencies() {
    # 一个APP的书写格式是："包管理器:包名"
    # 例如：
    # "pacman:vim"
    # "yay:vim"
...
function pywal::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function pywal::trait::main() {
    return "${SHELL_TRUE}"
}
