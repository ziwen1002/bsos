...
function wlogout::trait::post_install() {
    ...
    hyprland::config::add "350" "$SCRIPT_DIR_37160405/wlogout.conf" || return "$SHELL_FALSE"
    return "${SHELL_TRUE}"
}
...
function wlogout::trait::post_uninstall() {
    ...
    hyprland::config::remove "350" "wlogout.conf" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function wlogout::trait::upgrade() {
    package_manager::upgrade "$(wlogout::trait::package_manager)" "$(wlogout::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
function wlogout::trait::dependencies() {
    ...
    local apps=()
    # 动态颜色主题
    apps+=("custom:wallust" "pacman:go-yq")
    array::print apps
    return "${SHELL_TRUE}"
}
