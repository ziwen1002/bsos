...
SCRIPT_DIR_a35d2bad="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
# 指定使用的包管理器
function cavasik::trait::package_manager() {
    echo "flatpak"
}

# 需要安装包的名称，如果安装一个应用需要安装多个包，那么这里填写最核心的包，其他的包算是依赖
function cavasik::trait::package_name() {
    echo "io.github.TheWisker.Cavasik"
}

# 简短的描述信息，查看包的信息的时候会显示
function cavasik::trait::description() {
    package_manager::package_description "$(cavasik::trait::package_manager)" "$(cavasik::trait::package_name)" || return "$SHELL_FALSE"
    return "$SHELL_TRUE"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function cavasik::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function cavasik::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的前置操作，比如卸载依赖
function cavasik::trait::pre_uninstall() {
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function cavasik::trait::upgrade() {
    package_manager::upgrade "$(cavasik::trait::package_manager)" "$(cavasik::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 如果直接卸载也不会有残留就不用处理
function cavasik::trait::unfixme() {
    return "${SHELL_TRUE}"
}
...
function cavasik::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function cavasik::trait::main() {
    return "${SHELL_TRUE}"
}
