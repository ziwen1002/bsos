...
SCRIPT_DIR_6007d639="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function wezterm::trait::description() {
    package_manager::package_description "$(wezterm::trait::package_manager)" "$(wezterm::trait::package_name)"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function wezterm::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function wezterm::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
function wezterm::trait::post_install() {
    ...
    if os::is_not_vm; then
        hyprctl::config::add "${SCRIPT_DIR_6007d639}/350-wezterm.conf" || return "${SHELL_FALSE}"
    fi
...
# 卸载的前置操作，比如卸载依赖
function wezterm::trait::pre_uninstall() {
    return "${SHELL_TRUE}"
}
...
function wezterm::trait::post_uninstall() {
    ...
    hyprctl::config::remove "350-wezterm.conf" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 如果直接卸载也不会有残留就不用处理
function wezterm::trait::unfixme() {
    return "${SHELL_TRUE}"
}
...
function wezterm::trait::dependencies() {
    # 一个APP的书写格式是："包管理器:包名"
    # 例如：
    # "pacman:vim"
    # "yay:vim"
    ...
    local apps=("custom:fonts" "custom:zsh")
    array::print apps
    return "${SHELL_TRUE}"
}
...
function wezterm::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function wezterm::trait::main() {
    return "$SHELL_TRUE"
}
