...
SCRIPT_DIR_dd866f74="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
# 指定使用的包管理器
function locale::trait::package_manager() {
    echo "pacman"
}

# 需要安装包的名称，如果安装一个应用需要安装多个包，那么这里填写最核心的包，其他的包算是依赖
function locale::trait::package_name() {
    echo "locale"
}

# 简短的描述信息，查看包的信息的时候会显示
function locale::trait::description() {
    package_manager::package_description "$(locale::trait::package_manager)" "$(locale::trait::package_name)" || return "$SHELL_FALSE"
    return "$SHELL_TRUE"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function locale::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function locale::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的前置操作，比如卸载依赖
function locale::trait::pre_uninstall() {
    return "${SHELL_TRUE}"
}
...
function locale::trait::dependencies() {
    # 一个APP的书写格式是："包管理器:包名"
    # 例如：
    # "pacman:vim"
    # "yay:vim"
...
function locale::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function locale::trait::main() {
    return "${SHELL_TRUE}"
}
