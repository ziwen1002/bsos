...
SCRIPT_DIR_b8483b18="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
# 指定使用的包管理器
function hyprlock::trait::package_manager() {
    echo "pacman"
}

# 需要安装包的名称，如果安装一个应用需要安装多个包，那么这里填写最核心的包，其他的包算是依赖
function hyprlock::trait::package_name() {
    echo "hyprlock"
}

# 简短的描述信息，查看包的信息的时候会显示
function hyprlock::trait::description() {
    package_manager::package_description "$(hyprlock::trait::package_manager)" "$(hyprlock::trait::package_name)" || return "$SHELL_FALSE"
    return "$SHELL_TRUE"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function hyprlock::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function hyprlock::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的前置操作，比如卸载依赖
function hyprlock::trait::pre_uninstall() {
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function hyprlock::trait::upgrade() {
    package_manager::upgrade "$(hyprlock::trait::package_manager)" "$(hyprlock::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 如果直接卸载也不会有残留就不用处理
function hyprlock::trait::unfixme() {
    return "${SHELL_TRUE}"
}
...
function hyprlock::trait::dependencies() {
    # 一个APP的书写格式是："包管理器:包名"
    # 例如：
    # "pacman:vim"
    # "yay:vim"
...
function hyprlock::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function hyprlock::trait::main() {
    return "${SHELL_TRUE}"
}
