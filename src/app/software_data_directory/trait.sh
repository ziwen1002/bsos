...
# - 更新的指责和包管理器类似，只负责更新
function software_data_directory::trait::upgrade() {
    # package_manager::upgrade "$(software_data_directory::trait::package_manager)" "$(software_data_directory::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
