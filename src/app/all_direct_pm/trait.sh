...
# 安装的操作
function all_direct_pm::trait::install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的操作
function all_direct_pm::trait::uninstall() {
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function all_direct_pm::trait::upgrade() {
    # package_manager::upgrade "$(all_direct_pm::trait::package_manager)" "$(all_direct_pm::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
