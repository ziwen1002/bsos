...
SCRIPT_DIR_14dfd37a="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function vmware_workstation::trait::package_name() {
    echo "vmware-workstation"
}

# 简短的描述信息，查看包的信息的时候会显示
function vmware_workstation::trait::description() {
    package_manager::package_description "$(vmware_workstation::trait::package_manager)" "$(vmware_workstation::trait::package_name)"
}

# 安装向导，和用户交互相关的，然后将得到的结果写入配置
# 后续安装的时候会用到的配置
function vmware_workstation::trait::install_guide() {
    return "${SHELL_TRUE}"
}

# 安装的前置操作，比如下载源代码
function vmware_workstation::trait::pre_install() {
    return "${SHELL_TRUE}"
}
...
# 卸载的后置操作，比如删除临时文件
function vmware_workstation::trait::post_uninstall() {
    return "${SHELL_TRUE}"
}
...
# - 更新的指责和包管理器类似，只负责更新
function vmware_workstation::trait::upgrade() {
    package_manager::upgrade "$(vmware_workstation::trait::package_manager)" "$(vmware_workstation::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
...
# 如果直接卸载也不会有残留就不用处理
function vmware_workstation::trait::unfixme() {
    return "${SHELL_TRUE}"
}
...
function vmware_workstation::trait::dependencies() {
    # 一个APP的书写格式是："包管理器:包名"
    # 例如：
    # "pacman:vim"
    # "yay:vim"
    ...
    local apps=("pacman:linux-headers")
    array::print apps
    return "${SHELL_TRUE}"
}
...
function vmware_workstation::trait::features() {
    local apps=()
    array::print apps
    return "${SHELL_TRUE}"
}

function vmware_workstation::trait::main() {
    return "$SHELL_TRUE"
}

vmware_workstation::trait::main
