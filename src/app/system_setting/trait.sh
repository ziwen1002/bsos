...
# - 更新的指责和包管理器类似，只负责更新
function system_setting::trait::upgrade() {
    # package_manager::upgrade "$(system_setting::trait::package_manager)" "$(system_setting::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
