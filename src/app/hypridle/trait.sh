...
function hypridle::trait::post_install() {
    ...
    if os::is_vm; then
        ...
        hyprland::config::add "350" "${SCRIPT_DIR_251802b6}/hypridle.conf" || return "${SHELL_FALSE}"
    fi
...
# - 更新的指责和包管理器类似，只负责更新
function hypridle::trait::upgrade() {
    package_manager::upgrade "$(hypridle::trait::package_manager)" "$(hypridle::trait::package_name)" || return "${SHELL_FALSE}"
    return "${SHELL_TRUE}"
}
