...
function template::_clean_build() {
...
function template::package_name() {
    template::trait::package_name
}
...
function template::dependencies() {
    template::trait::dependencies
}

function template::features() {
    template::trait::features
}

function template::install_guide() {
    template::trait::install_guide
}

function template::finally() {
    template::trait::finally
}

function template::main() {

    template::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_uuid}/trait.sh" || return "$SHELL_FALSE"
