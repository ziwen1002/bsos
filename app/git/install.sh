...
SCRIPT_DIR_e7924e27="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function git::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_e7924e27}")"
...
function git::package_name() {
    git::trait::package_name
}

function git::description() {
    git::trait::description
}

function git::dependencies() {
    git::trait::dependencies
}

function git::features() {
    git::trait::features
}

function git::install_guide() {
    git::trait::install_guide
}

function git::finally() {
    git::trait::finally
}
...
function git::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_e7924e27}/trait.sh" || return "$SHELL_FALSE"
