...
SCRIPT_DIR_0f2e0abf="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function anyrun::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_0f2e0abf}")"
...
function anyrun::package_name() {
    anyrun::trait::package_name
}

function anyrun::description() {
    anyrun::trait::description
}

function anyrun::dependencies() {
    anyrun::trait::dependencies
}

function anyrun::features() {
    anyrun::trait::features
}

function anyrun::install_guide() {
    anyrun::trait::install_guide
}

function anyrun::finally() {
    anyrun::trait::finally
}

function anyrun::main() {

    anyrun::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_0f2e0abf}/trait.sh" || return "$SHELL_FALSE"
