...
SCRIPT_DIR_3473f22d="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function fzf::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_3473f22d}")"
...
function fzf::package_name() {
    fzf::trait::package_name
}

function fzf::description() {
    fzf::trait::description
}

function fzf::dependencies() {
    fzf::trait::dependencies
}

function fzf::features() {
    fzf::trait::features
}

function fzf::install_guide() {
    fzf::trait::install_guide
}

function fzf::finally() {
    fzf::trait::finally
}
...
function fzf::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_3473f22d}/trait.sh" || return "$SHELL_FALSE"
