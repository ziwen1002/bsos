...
SCRIPT_DIR_8b476897="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function zsh_syntax_highlighting::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_8b476897}")"
...
function zsh_syntax_highlighting::_clean_build() {
...
function zsh_syntax_highlighting::_install() {

    linfo "clean app(${PM_APP_NAME}) build env..."
    zsh_syntax_highlighting::_clean_build || return "$SHELL_FALSE"
    linfo "clean app(${PM_APP_NAME}) build env success."

    linfo "pre install app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::pre_install || return "$SHELL_FALSE"
    linfo "pre install app(${PM_APP_NAME}) success."

    linfo "do install app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::do_install || return "$SHELL_FALSE"
    linfo "do install app(${PM_APP_NAME}) success."

    linfo "post install app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::post_install || return "$SHELL_FALSE"
...
function zsh_syntax_highlighting::install() {
    zsh_syntax_highlighting::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        zsh_syntax_highlighting::uninstall
...
# 2. 可以反复执行
function zsh_syntax_highlighting::_uninstall() {
    linfo "pre uninstall app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::pre_uninstall || return "$SHELL_FALSE"
    linfo "pre uninstall app(${PM_APP_NAME}) success."

    linfo "do uninstall app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::do_uninstall || return "$SHELL_FALSE"
    linfo "do uninstall app(${PM_APP_NAME}) success."

    linfo "post uninstall app(${PM_APP_NAME})..."
    zsh_syntax_highlighting::trait::post_uninstall || return "$SHELL_FALSE"
...
function zsh_syntax_highlighting::uninstall() {
    zsh_syntax_highlighting::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function zsh_syntax_highlighting::package_name() {
    zsh_syntax_highlighting::trait::package_name
}

function zsh_syntax_highlighting::description() {
    zsh_syntax_highlighting::trait::description
}

function zsh_syntax_highlighting::dependencies() {
    zsh_syntax_highlighting::trait::dependencies
}

function zsh_syntax_highlighting::features() {
    zsh_syntax_highlighting::trait::features
}

function zsh_syntax_highlighting::install_guide() {
    zsh_syntax_highlighting::trait::install_guide
}

function zsh_syntax_highlighting::finally() {
    zsh_syntax_highlighting::trait::finally
}

function zsh_syntax_highlighting::main() {

    zsh_syntax_highlighting::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_8b476897}/trait.sh" || return "$SHELL_FALSE"
    ...
    "install")
        zsh_syntax_highlighting::install
        return $?
        ;;
    ...
    "package_name")
        zsh_syntax_highlighting::package_name
        return $?
        ;;

    "description")
        zsh_syntax_highlighting::description
        return $?
        ;;

    "dependencies")
        zsh_syntax_highlighting::dependencies
        return $?
        ;;

    "features")
        zsh_syntax_highlighting::features
        return $?
        ;;

    "install_guide")
        zsh_syntax_highlighting::install_guide
        return $?
        ;;

    "finally")
        zsh_syntax_highlighting::finally
        return $?
        ;;
    *)
