...
SCRIPT_DIR_a3807335="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function pkgfile::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_a3807335}")"
...
function pkgfile::package_name() {
    pkgfile::trait::package_name
}

function pkgfile::description() {
    pkgfile::trait::description
}

function pkgfile::dependencies() {
    pkgfile::trait::dependencies
}

function pkgfile::features() {
    pkgfile::trait::features
}

function pkgfile::install_guide() {
    pkgfile::trait::install_guide
}

function pkgfile::finally() {
    pkgfile::trait::finally
}

function pkgfile::main() {

    pkgfile::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_a3807335}/trait.sh" || return "$SHELL_FALSE"
