...
SCRIPT_DIR_b1269d35="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function pot::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_b1269d35}")"
...
function pot::package_name() {
    pot::trait::package_name
}

function pot::description() {
    pot::trait::description
}

function pot::dependencies() {
    pot::trait::dependencies
}

function pot::features() {
    pot::trait::features
}

function pot::install_guide() {
    pot::trait::install_guide
}

function pot::finally() {
    pot::trait::finally
}
...
function pot::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_b1269d35}/trait.sh" || return "$SHELL_FALSE"
