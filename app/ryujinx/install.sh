...
SCRIPT_DIR_bf1f2218="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function ryujinx::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_bf1f2218}")"
...
function ryujinx::package_name() {
    ryujinx::trait::package_name
}

function ryujinx::description() {
    ryujinx::trait::description
}

function ryujinx::dependencies() {
    ryujinx::trait::dependencies
}

function ryujinx::features() {
    ryujinx::trait::features
}

function ryujinx::install_guide() {
    ryujinx::trait::install_guide
}

function ryujinx::finally() {
    ryujinx::trait::finally
}

function ryujinx::main() {

    ryujinx::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_bf1f2218}/trait.sh" || return "$SHELL_FALSE"
