...
SCRIPT_DIR_da2a3c86="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function ssh::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_da2a3c86}")"
...
function ssh::package_name() {
    ssh::trait::package_name
}

function ssh::description() {
    ssh::trait::description
}

function ssh::dependencies() {
    ssh::trait::dependencies
}

function ssh::features() {
    ssh::trait::features
}

function ssh::install_guide() {
    ssh::trait::install_guide
}

function ssh::finally() {
    ssh::trait::finally
}
...
function ssh::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_da2a3c86}/trait.sh" || return "$SHELL_FALSE"
