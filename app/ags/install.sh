...
SCRIPT_DIR_c19cc48f="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function ags::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_c19cc48f}")"
...
function ags::package_name() {
    ags::trait::package_name
}

function ags::description() {
    ags::trait::description
}

function ags::dependencies() {
    ags::trait::dependencies
}

function ags::features() {
    ags::trait::features
}

function ags::install_guide() {
    ags::trait::install_guide
}

function ags::finally() {
    ags::trait::finally
}
...
function ags::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_c19cc48f}/trait.sh" || return "$SHELL_FALSE"
