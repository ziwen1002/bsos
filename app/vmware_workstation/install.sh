...
SCRIPT_DIR_8f09f329="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function vmware_workstation::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_8f09f329}")"
...
function vmware_workstation::_clean_build() {
...
function vmware_workstation::install() {
    vmware_workstation::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        vmware_workstation::uninstall
...
# 2. 可以反复执行
function vmware_workstation::_uninstall() {
    linfo "pre uninstall app(${PM_APP_NAME})..."
    vmware_workstation::trait::pre_uninstall || return "$SHELL_FALSE"
    linfo "pre uninstall app(${PM_APP_NAME}) success."

    linfo "do uninstall app(${PM_APP_NAME})..."
    vmware_workstation::trait::do_uninstall || return "$SHELL_FALSE"
    linfo "do uninstall app(${PM_APP_NAME}) success."

    linfo "post uninstall app(${PM_APP_NAME})..."
    vmware_workstation::trait::post_uninstall || return "$SHELL_FALSE"
...
function vmware_workstation::uninstall() {
    vmware_workstation::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function vmware_workstation::package_name() {
    vmware_workstation::trait::package_name
}

function vmware_workstation::description() {
    vmware_workstation::trait::description
}

function vmware_workstation::dependencies() {
    vmware_workstation::trait::dependencies
}

function vmware_workstation::features() {
    vmware_workstation::trait::features
}

function vmware_workstation::install_guide() {
    vmware_workstation::trait::install_guide
}

function vmware_workstation::finally() {
    vmware_workstation::trait::finally
}

function vmware_workstation::main() {

    vmware_workstation::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_8f09f329}/trait.sh" || return "$SHELL_FALSE"
    ...
    "package_name")
        vmware_workstation::package_name
        return $?
        ;;

    "description")
        vmware_workstation::description
        return $?
        ;;

    "dependencies")
        vmware_workstation::dependencies
        return $?
        ;;

    "features")
        vmware_workstation::features
        return $?
        ;;

    "install_guide")
        vmware_workstation::install_guide
        return $?
        ;;

    "finally")
        vmware_workstation::finally
        return $?
        ;;
    *)
