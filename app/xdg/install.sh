...
SCRIPT_DIR_9cdd3691="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function xdg::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_9cdd3691}")"
...
function xdg::package_name() {
    xdg::trait::package_name
}

function xdg::description() {
    xdg::trait::description
}

function xdg::dependencies() {
    xdg::trait::dependencies
}

function xdg::features() {
    xdg::trait::features
}

function xdg::install_guide() {
    xdg::trait::install_guide
}

function xdg::finally() {
    xdg::trait::finally
}
...
function xdg::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_9cdd3691}/trait.sh" || return "$SHELL_FALSE"
