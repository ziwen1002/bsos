...
SCRIPT_DIR_3d83bc8e="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function polkit::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_3d83bc8e}")"
...
function polkit::package_name() {
    polkit::trait::package_name
}

function polkit::description() {
    polkit::trait::description
}

function polkit::dependencies() {
    polkit::trait::dependencies
}

function polkit::features() {
    polkit::trait::features
}

function polkit::install_guide() {
    polkit::trait::install_guide
}

function polkit::finally() {
    polkit::trait::finally
}

function polkit::main() {

    polkit::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_3d83bc8e}/trait.sh" || return "$SHELL_FALSE"
