...
SCRIPT_DIR_9a6fae1b="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function zsh_autosuggestions::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_9a6fae1b}")"
...
function zsh_autosuggestions::_clean_build() {
...
function zsh_autosuggestions::install() {
    zsh_autosuggestions::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        zsh_autosuggestions::uninstall
...
# 2. 可以反复执行
function zsh_autosuggestions::_uninstall() {
    linfo "pre uninstall app(${PM_APP_NAME})..."
    zsh_autosuggestions::trait::pre_uninstall || return "$SHELL_FALSE"
    linfo "pre uninstall app(${PM_APP_NAME}) success."

    linfo "do uninstall app(${PM_APP_NAME})..."
    zsh_autosuggestions::trait::do_uninstall || return "$SHELL_FALSE"
    linfo "do uninstall app(${PM_APP_NAME}) success."

    linfo "post uninstall app(${PM_APP_NAME})..."
    zsh_autosuggestions::trait::post_uninstall || return "$SHELL_FALSE"
...
function zsh_autosuggestions::uninstall() {
    zsh_autosuggestions::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function zsh_autosuggestions::package_name() {
    zsh_autosuggestions::trait::package_name
}

function zsh_autosuggestions::description() {
    zsh_autosuggestions::trait::description
}

function zsh_autosuggestions::dependencies() {
    zsh_autosuggestions::trait::dependencies
}

function zsh_autosuggestions::features() {
    zsh_autosuggestions::trait::features
}

function zsh_autosuggestions::install_guide() {
    zsh_autosuggestions::trait::install_guide
}

function zsh_autosuggestions::finally() {
    zsh_autosuggestions::trait::finally
}

function zsh_autosuggestions::main() {

    zsh_autosuggestions::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_9a6fae1b}/trait.sh" || return "$SHELL_FALSE"
    ...
    "package_name")
        zsh_autosuggestions::package_name
        return $?
        ;;

    "description")
        zsh_autosuggestions::description
        return $?
        ;;

    "dependencies")
        zsh_autosuggestions::dependencies
        return $?
        ;;

    "features")
        zsh_autosuggestions::features
        return $?
        ;;

    "install_guide")
        zsh_autosuggestions::install_guide
        return $?
        ;;

    "finally")
        zsh_autosuggestions::finally
        return $?
        ;;
    *)
