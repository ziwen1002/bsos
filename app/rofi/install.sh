...
SCRIPT_DIR_383f5515="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function rofi::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_383f5515}")"
...
function rofi::package_name() {
    rofi::trait::package_name
}

function rofi::description() {
    rofi::trait::description
}

function rofi::dependencies() {
    rofi::trait::dependencies
}

function rofi::features() {
    rofi::trait::features
}

function rofi::install_guide() {
    rofi::trait::install_guide
}

function rofi::finally() {
    rofi::trait::finally
}
...
function rofi::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_383f5515}/trait.sh" || return "$SHELL_FALSE"
