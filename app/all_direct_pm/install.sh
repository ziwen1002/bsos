...
SCRIPT_DIR_c69e34e5="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function all_direct_pm::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_c69e34e5}")"
...
function all_direct_pm::_clean_build() {
...
function all_direct_pm::install() {
    all_direct_pm::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        all_direct_pm::uninstall
...
function all_direct_pm::uninstall() {
    all_direct_pm::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function all_direct_pm::package_name() {
    all_direct_pm::trait::package_name
}

function all_direct_pm::description() {
    all_direct_pm::trait::description
}

function all_direct_pm::dependencies() {
    all_direct_pm::trait::dependencies
}

function all_direct_pm::features() {
    all_direct_pm::trait::features
}

function all_direct_pm::install_guide() {
    all_direct_pm::trait::install_guide
}

function all_direct_pm::finally() {
    all_direct_pm::trait::finally
}

function all_direct_pm::main() {

    all_direct_pm::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_c69e34e5}/trait.sh" || return "$SHELL_FALSE"
    ...
    "package_name")
        all_direct_pm::package_name
        return $?
        ;;

    "description")
        all_direct_pm::description
        return $?
        ;;

    "dependencies")
        all_direct_pm::dependencies
        return $?
        ;;

    "features")
        all_direct_pm::features
        return $?
        ;;

    "install_guide")
        all_direct_pm::install_guide
        return $?
        ;;

    "finally")
        all_direct_pm::finally
        return $?
        ;;
    *)
