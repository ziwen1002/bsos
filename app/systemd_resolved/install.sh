...
SCRIPT_DIR_80d928c9="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function systemd_resolved::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_80d928c9}")"
...
function systemd_resolved::_clean_build() {
...
function systemd_resolved::install() {
    systemd_resolved::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        systemd_resolved::uninstall
...
# 2. 可以反复执行
function systemd_resolved::_uninstall() {
    linfo "pre uninstall app(${PM_APP_NAME})..."
    systemd_resolved::trait::pre_uninstall || return "$SHELL_FALSE"
    linfo "pre uninstall app(${PM_APP_NAME}) success."

    linfo "do uninstall app(${PM_APP_NAME})..."
    systemd_resolved::trait::do_uninstall || return "$SHELL_FALSE"
    linfo "do uninstall app(${PM_APP_NAME}) success."

    linfo "post uninstall app(${PM_APP_NAME})..."
    systemd_resolved::trait::post_uninstall || return "$SHELL_FALSE"
...
function systemd_resolved::uninstall() {
    systemd_resolved::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function systemd_resolved::package_name() {
    systemd_resolved::trait::package_name
}

function systemd_resolved::description() {
    systemd_resolved::trait::description
}

function systemd_resolved::dependencies() {
    systemd_resolved::trait::dependencies
}

function systemd_resolved::features() {
    systemd_resolved::trait::features
}

function systemd_resolved::install_guide() {
    systemd_resolved::trait::install_guide
}

function systemd_resolved::finally() {
    systemd_resolved::trait::finally
}

function systemd_resolved::main() {

    systemd_resolved::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_80d928c9}/trait.sh" || return "$SHELL_FALSE"
    ...
    "package_name")
        systemd_resolved::package_name
        return $?
        ;;

    "description")
        systemd_resolved::description
        return $?
        ;;

    "dependencies")
        systemd_resolved::dependencies
        return $?
        ;;

    "features")
        systemd_resolved::features
        return $?
        ;;

    "install_guide")
        systemd_resolved::install_guide
        return $?
        ;;

    "finally")
        systemd_resolved::finally
        return $?
        ;;
    *)
