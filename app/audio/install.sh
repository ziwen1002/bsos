...
SCRIPT_DIR_f09921bd="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function audio::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_f09921bd}")"
...
function audio::package_name() {
    audio::trait::package_name
}

function audio::description() {
    audio::trait::description
}

function audio::dependencies() {
    audio::trait::dependencies
}

function audio::features() {
    audio::trait::features
}

function audio::install_guide() {
    audio::trait::install_guide
}

function audio::finally() {
    audio::trait::finally
}
...
function audio::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_f09921bd}/trait.sh" || return "$SHELL_FALSE"
