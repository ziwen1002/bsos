...
SCRIPT_DIR_d76c8806="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function amd_gpu::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_d76c8806}")"
...
function amd_gpu::package_name() {
    amd_gpu::trait::package_name
}

function amd_gpu::description() {
    amd_gpu::trait::description
}

function amd_gpu::dependencies() {
    amd_gpu::trait::dependencies
}

function amd_gpu::features() {
    amd_gpu::trait::features
}

function amd_gpu::install_guide() {
    amd_gpu::trait::install_guide
}

function amd_gpu::finally() {
    amd_gpu::trait::finally
}

function amd_gpu::main() {

    amd_gpu::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_d76c8806}/trait.sh" || return "$SHELL_FALSE"
