...
SCRIPT_DIR_7c8e17df="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function grub::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_7c8e17df}")"
...
function grub::package_name() {
    grub::trait::package_name
}

function grub::description() {
    grub::trait::description
}

function grub::dependencies() {
    grub::trait::dependencies
}

function grub::features() {
    grub::trait::features
}

function grub::install_guide() {
    grub::trait::install_guide
}

function grub::finally() {
    grub::trait::finally
}
...
function grub::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_7c8e17df}/trait.sh" || return "$SHELL_FALSE"
