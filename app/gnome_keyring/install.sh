...
SCRIPT_DIR_989cd49a="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function gnome_keyring::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_989cd49a}")"
...
function gnome_keyring::_clean_build() {
...
function gnome_keyring::install() {
    gnome_keyring::_install
    if [ $? -ne "${SHELL_TRUE}" ]; then
        lerror "install app(${PM_APP_NAME}) failed"
        linfo "clean app(${PM_APP_NAME})..."
        gnome_keyring::uninstall
...
function gnome_keyring::uninstall() {
    gnome_keyring::_uninstall
    if [ $? -ne "${SHELL_TRUE}" ]; then
...
function gnome_keyring::package_name() {
    gnome_keyring::trait::package_name
}

function gnome_keyring::description() {
    gnome_keyring::trait::description
}

function gnome_keyring::dependencies() {
    gnome_keyring::trait::dependencies
}

function gnome_keyring::features() {
    gnome_keyring::trait::features
}

function gnome_keyring::install_guide() {
    gnome_keyring::trait::install_guide
}

function gnome_keyring::finally() {
    gnome_keyring::trait::finally
}

function gnome_keyring::main() {

    gnome_keyring::_env || return 1

    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_989cd49a}/trait.sh" || return "$SHELL_FALSE"
    ...
    "package_name")
        gnome_keyring::package_name
        return $?
        ;;

    "description")
        gnome_keyring::description
        return $?
        ;;

    "dependencies")
        gnome_keyring::dependencies
        return $?
        ;;

    "features")
        gnome_keyring::features
        return $?
        ;;

    "install_guide")
        gnome_keyring::install_guide
        return $?
        ;;

    "finally")
        gnome_keyring::finally
        return $?
        ;;
    *)
