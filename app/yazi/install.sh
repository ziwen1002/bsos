...
SCRIPT_DIR_ca8e4101="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function yazi::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_ca8e4101}")"
...
function yazi::package_name() {
    yazi::trait::package_name
}

function yazi::description() {
    yazi::trait::description
}

function yazi::dependencies() {
    yazi::trait::dependencies
}

function yazi::features() {
    yazi::trait::features
}

function yazi::install_guide() {
    yazi::trait::install_guide
}

function yazi::finally() {
    yazi::trait::finally
}
...
function yazi::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_ca8e4101}/trait.sh" || return "$SHELL_FALSE"
