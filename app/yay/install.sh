...
SCRIPT_DIR_332277cc="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
...
function yay::_env() {
    ...
    # export PM_APP_NAME
    local name
    name="$(basename "${SCRIPT_DIR_332277cc}")"
...
function yay::package_name() {
    yay::trait::package_name
}

function yay::description() {
    yay::trait::description
}

function yay::dependencies() {
    yay::trait::dependencies
}

function yay::features() {
    yay::trait::features
}

function yay::install_guide() {
    yay::trait::install_guide
}

function yay::finally() {
    yay::trait::finally
}
...
function yay::main() {
    ...
    # shellcheck source=/dev/null
    source "${SCRIPT_DIR_332277cc}/trait.sh" || return "$SHELL_FALSE"
